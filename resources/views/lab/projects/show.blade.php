        ...
        <div class="lenticular-panel">
            ...
            @if($source?->metadata)
                ...
                <form method="post" action="{{ route('lab.projects.frames.store', ['locale' => app()->getLocale(), 'project' => $project]) }}" class="lenticular-controls">
                    @csrf
                    <input id="z-center" name="z_center" type="hidden" value="0.5">
                    <div class="lenticular-alignment-editor">
                        <div class="lenticular-vertical-control">
                            <output data-range-output="alignment-y">50%</output>
                        ...
                        <div class="lenticular-alignment-preview">
                            <div class="lenticular-alignment-stage" data-alignment-stage>
                                ...
                                @endforeach
                                <button class="lenticular-z-zone" type="button" data-z-zone aria-label="{{ __('lenticular_projects.move_z') }}" aria-valuemin="0" aria-valuemax="100" aria-valuenow="50"><span data-alignment-line></span></button>
                            </div>
                            <div class="lenticular-width-control">
                                <label for="z-width">{{ __('lenticular_projects.z_width') }} <output data-range-output="z-width">5%</output></label>
                                <input id="z-width" name="z_width" type="range" min="0.01" max="0.5" step="0.01" value="0.05">
                            </div>
                        </div>
