...
class CommerceSettingsService
{
    ...
    {
        return $this->bool('paynow.enabled', false)
            && filled($this->payNowApiKey())
            && filled($this->payNowSignatureKey());
    }

    public function payNowSandbox(): bool
    {
        return $this->bool('paynow.sandbox', true);
    }

    public function payNowApiKey(): ?string
    {
        return $this->get('paynow.api_key');
    }

    public function payNowSignatureKey(): ?string
    {
        return $this->get('paynow.signature_key');
    }
    ...
    public function payNowTimeout(): int
    {
        return max(
            3,
            min(60, $this->int('paynow.timeout', 15))
        );
    }
    ...
    {
        return [
            'recipient' => $this->get('bank.recipient'),
            'bank_name' => $this->get('bank.name'),
            'account' => $this->get('bank.account'),
            'swift' => $this->get('bank.swift'),
        ];
    }
    ...
    public function seller(): array
    {
        return [
            ...
            'address' => $this->get('seller.address'),
            'tax_id' => $this->get('seller.tax_id'),
            'email' => $this->get('seller.email'),
        ];
    }
